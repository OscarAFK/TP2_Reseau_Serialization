...
namespace uqac {
	namespace utilsTP2 {
		...
		Player::Player(utils::Vector3 position, utils::Vector3 taille, utils::Quaternion rotation, int vie, int armure, float argent, char nom[128]) :
		...
		void Player::Write(serialization::Serializer* s)
		{
			...
			serialization::Vector3_32 tailleComp = comp.compressVector3_32(m_taille, -500, 500, -500, 500, 0, 100, 3);
		...
		void Player::Read(serialization::Deserializer* d)
		{
			serialization::Compressor comp;
			...
			serialization::Vector3_32 tailleComp = serialization::Vector3_32(tailleXComp, tailleYComp, tailleZComp);
